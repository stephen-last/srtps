...
function getPoints(x, y, w, h) {
  ...
  var deg1 = Math.PI - Math.acos(w / 2 / r);
  var deg2 = Math.acos(w / 2 / r);
  var deg3 = -Math.acos(w / 2 / r);
  var deg4 = Math.PI + Math.acos(w / 2 / r);
  return [{ x: c.x + r * Math.cos(deg1), y: c.y - r * Math.sin(deg1) }, { x: c.x + r * Math.cos(deg2), y: c.y - r * Math.sin(deg2) }, { x: c.x + r * Math.cos(deg3), y: c.y - r * Math.sin(deg3) }, { x: c.x + r * Math.cos(deg4), y: c.y - r * Math.sin(deg4) }];
}
...
function getRectPaths(rects) {
  ...
  var paths = nodes.map(function (node) {
    var points = getPoints(node.x, node.y, node.w, node.h);
